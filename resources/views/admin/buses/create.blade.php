...
<form action="{{ route('admin.buses.store') }}" method="POST">
    ...
    <select name="loai_xe" id="loai_xe" class="form-control">
        ...
        <option value="Giường" {{ old('loai_xe', $bus->loai_xe ?? '') == 'Giường' ? 'selected' : '' }}>Giường</option>
        <option value="Limousine" {{ old('loai_xe', $bus->loai_xe ?? '') == 'Limousine' ? 'selected' : '' }}>Limousine</option>
    </select>
    @error('loai_xe')
