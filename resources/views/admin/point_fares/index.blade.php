                        ...
                        <select name="pickup_point_id" id="pickup_point_id" class="form-select">
                            ...
                            @foreach ($pickupPoints as $point)
                                <option value="{{ $point->id }}"
                                    {{ request('pickup_point_id') == $point->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="dropoff_point_id" id="dropoff_point_id" class="form-select">
                            ...
                            @foreach ($dropoffPoints as $point)
                                <option value="{{ $point->id }}"
                                    {{ request('dropoff_point_id') == $point->id ? 'selected' : '' }}>
                                    {{ $point->name }} ({{ $point->city?->name }})
                                </option>
                            @endforeach
                        </select>
                            ...
                            @forelse($pointFares as $pointFare)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <span
                                            class="d-block small text-muted">({{ $pointFare->route->name ?? 'Tuyến #' . $pointFare->route_id }})</span>
                                    </td>
                                    <td>
                                        <span
                                            class="d-block fw-semibold">{{ $pointFare->pickupPoint->name ?? '---' }}</span>
                                        <span
                                            class="d-block small text-muted">{{ $pointFare->pickupPoint->city?->name ?? '---' }}</span>
                                    </td>
                                    <td>
                                        <span
                                            class="d-block fw-semibold">{{ $pointFare->dropoffPoint->name ?? '---' }}</span>
                                        <span
                                            class="d-block small text-muted">{{ $pointFare->dropoffPoint->city?->name ?? '---' }}</span>
                                    </td>
                                    <td class="fw-semibold text-success">
                                        {{ number_format($pointFare->price, 0, ',', '.') }} VNĐ
                                    </td>
                                    <td>
                                        @if ($pointFare->status == 1)
                                    ...
                                    <td class="text-center">
                                        ...
                                        <form action="{{ route('admin.point_fares.destroy', $pointFare->id) }}"
                                            method="POST" class="d-inline-block delete-point-fare-form">
                                            @csrf
                                            @method('DELETE')
